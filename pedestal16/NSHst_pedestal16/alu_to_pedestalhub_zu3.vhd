----------------------------------------------------------------------------------
-- Create Date: 05/28/2026 10:21:58 PM
-- Module Name: alu_to_pedestalhub_zu3 - Behavioral
-- Target Devices: XCZU3EG-2SFVC784E
-- Tool Versions: vivado 25.2
-- Description: 
-- Convert ALU e9 f13 to pedestal HUB e8 f7 with zero & NAR, do saturation
-- ALU register: z/nzr, sign, exp(8..0), msb, fract(11..1), hlf-bit
-- Pedestal: sign, exp(7..0), fract(6...0) implicit leading 1.0 bit and implicit trailing half-bit
-- Revision:
...
alu_fract <= '1' & unsigned(alureg(10 downto 0)) & '1'; -- prefix leading one bit, suffux half-bit
...
--      combinatorial logic
if zero_nar = '1' then mem_exp <= "00000000"; mem_fract<= "0000000"; end if;        -- inset zero/NaR
if alu_exp > "011111111" then mem_exp <= "01111111"; mem_fract<= "1111111"; end if; -- saturate if needed
end process;
